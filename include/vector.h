...
/* Assigns data pointed to by data to position pos inside Vector vector. Keep in mind that the maximum value of pos must be vector->count - 1.
 * Return value:
 * on success: 0,
 ...
 * Performs a call to vec_assign() to assign value to position of new element.
 * Return value:
 * on success: 0
 * on failure: one of the error codes above. */
...
#define VEC_REMOVE_ERR_SHIFTING_OP_FAILED (VEC_REMOVE_ERR_BASE + 3) // internal function _vec_shift_right(struct Vector* vector, size_t start_idx) failed.
...
#define VEC_POP_ERR_BASE 50

#define VEC_POP_ERR_NULL_VEC (VEC_POP_ERR_BASE + 1) // argument vector equals to NULL
#define VEC_POP_ERR_POS_OUT_OF_BOUNDS (VEC_POP_ERR_BASE + 2) // arugment pos is of greater value than vector->count.
